// src/js/ui.js
// V19.3: Bug Fix - Steam Comparison Restored & ID Mismatch Fixed
...
// --- 样式常量 (V19.1 专业平衡版) ---
const INPUT_STYLE = "w-full px-4 py-2 bg-white border border-gray-300 rounded-lg text-lg font-bold text-gray-900 focus:border-blue-600 focus:ring-2 focus:ring-blue-600/20 transition-all outline-none placeholder-gray-400 h-12 shadow-sm";
const LABEL_STYLE = "block text-sm font-bold text-gray-600 mb-2 tracking-wide";
const GROUP_STYLE = "bg-white p-1 mb-6";
...
function generateProjectInputsHTML() {
    return `
        ...
        <div class="pt-6 border-t border-gray-200">
            <label class="${LABEL_STYLE} mb-4">计算模式</label>
                ...
                ${[
                    ...
                    <div class="flex items-center justify-between group related-to-${item.k} transition-all duration-200 p-2 hover:bg-gray-50 rounded-lg border border-transparent hover:border-gray-200">
                        <div class="flex items-center">
                            ...
                            <label for="compare_${item.k}" class="ml-3 text-base font-bold text-gray-700 cursor-pointer select-none">${item.n}</label>
                        </div>
                        <div class="flex items-center gap-3">
                            <span class="text-xs text-gray-400 font-bold hidden md:inline">投资(万)</span>
                            <div class="relative w-24">
                                <input type="number" id="${item.k}BoilerCapex" ${defAttr(defCapex)} class="w-full px-2 py-1.5 border border-gray-200 rounded-lg text-base font-bold text-right focus:border-blue-600 focus:ring-1 focus:ring-blue-600/20 outline-none transition-all text-gray-700" placeholder="0">
                            </div>
...
function generateOperatingInputsHTML() {
    return `
        <div>
            ...
            <input type="number" id="hpCop" ${defAttr(ENERGY_DEFAULTS.hp.cop)} step="0.1" class="${INPUT_STYLE} !text-2xl !text-blue-700" data-validation="isStrictlyPositive">
        </div>
        ...
        <div class="pt-6 border-t border-dashed border-gray-200 space-y-6">
             ...
             <div class="grid grid-cols-2 gap-4">
                 <div class="related-to-gas">
                    <label class="${LABEL_STYLE}">气价 (元/m³)</label>
                    <input type="number" id="gasPrice" ${defAttr(ENERGY_DEFAULTS.gas.price)} class="${INPUT_STYLE}">
                </div>
        ...
        <div class="pt-6 border-t border-dashed border-gray-200">
            <label class="${LABEL_STYLE} mb-4">锅炉效率 (%)</label>
            <div class="space-y-3">
                ${[
                    ...
                    <div class="flex items-center justify-between related-to-${item.id}">
                        <span class="text-base font-bold text-gray-600 w-12">${item.label}</span>
                        <div class="flex-1 flex items-center space-x-2">
                            <input type="number" id="${item.id}BoilerEfficiency" ${defAttr(item.val)} class="flex-1 px-3 py-2 bg-gray-50 border border-transparent rounded-lg text-lg font-bold text-gray-800 focus:bg-white focus:border-blue-500 transition-all text-center h-10">
                            <button type="button" class="eff-calc-btn text-blue-600 bg-blue-50 hover:bg-blue-100 text-xs font-bold px-3 py-2 rounded-lg border border-blue-100 transition-colors" data-target="${item.id}BoilerEfficiency" data-fuel="${item.id}">反推</button>
                        </div>
                    </div>
                `).join('')}
                <div class="hidden">
            ...
            <details class="group">
                ...
                <div class="text-gray-500 mt-4 grid grid-cols-1 gap-3 animate-fadeIn">
                    <div class="bg-gray-50 p-3 rounded-lg flex justify-between items-center border border-gray-200">
                        ...
                        <div class="flex items-center"><span class="text-xs mr-2 font-bold">排放</span><input id="gridFactor" type="number" ${defAttr(ENERGY_DEFAULTS.electric.factor)} class="w-20 p-1 border border-gray-300 rounded text-right font-bold text-gray-900 text-sm"><span class="ml-2 text-xs font-bold">kg/kWh</span></div>
                    </div>
                    ${['gas|气|m³', 'coal|煤|kg', 'fuel|油|kg', 'biomass|生物|kg', 'steam|蒸汽|kg'].map(item => {
                        const [k, n, u] = item.split('|');
                        return `
                        <div class="bg-gray-50 p-3 rounded-lg flex justify-between items-center border border-gray-200 related-to-${k}">
                            <span class="text-sm font-bold text-gray-700">${n} (${k})</span>
                            <div class="flex items-center gap-2">
                                <div class="flex items-center"><span class="text-xs mr-1 font-bold">热值</span><input id="${k}Calorific" type="number" ${defAttr(ENERGY_DEFAULTS[k].calorific)} class="w-16 p-1 border border-gray-300 rounded text-right font-bold text-gray-900 text-sm">
                                    <select id="${k}CalorificUnit" class="ml-1 p-1 border border-gray-300 rounded bg-white text-xs font-bold w-16 track-change">
                                        <option value="MJ" selected>MJ</option>
                                        <option value="kcal">kcal</option>
                                        <option value="kWh">kWh</option>
                                    </select>
                                </div>
                                <div class="flex items-center"><span class="text-xs mr-1 font-bold">排放</span><input id="${k}Factor" type="number" ${defAttr(ENERGY_DEFAULTS[k].factor)} class="w-16 p-1 border border-gray-300 rounded text-right font-bold text-gray-900 text-sm"></div>
                            </div>
                        </div>`;
                    }).join('')}
                </div>
        ...
        <div class="pt-6 border-t border-dashed border-gray-200">
            <label class="${LABEL_STYLE} mb-4">运维成本 (万/年)</label>
            <div class="space-y-4">
                <div class="flex items-center justify-between">
                    <span class="text-base font-bold text-blue-600 w-24">工业热泵</span>
                    <input type="number" id="hpOpexCost" ${defAttr(ENERGY_DEFAULTS.hp.opex)} class="flex-1 px-3 py-2 bg-blue-50 border border-blue-100 rounded-lg text-lg font-bold text-blue-800 text-center h-12 shadow-sm">
                </div>
                ${[
                    ...
                    <div class="flex items-center justify-between related-to-${item.id}">
                        <span class="text-base font-bold text-gray-600 w-24">${item.label}</span>
                        <input type="number" id="${item.id}OpexCost" ${defAttr(item.val)} class="flex-1 px-3 py-2 bg-gray-50 border border-transparent rounded-lg text-lg font-bold text-gray-700 text-center h-12">
                    </div>
                `).join('')}
            </div>
        </div>
...
function injectAccordionContent(containerId, title, contentHTML) {
    ...
    if (!container) return;
    container.innerHTML = `
        <button class="accordion-header flex justify-between items-center w-full px-4 py-4 bg-white hover:bg-gray-50 focus:outline-none transition-colors duration-200 cursor-pointer" type="button" aria-expanded="true" aria-controls="${containerId}-content">
        ...
        <div id="${containerId}-content" class="accordion-content open px-4 py-4 border-t border-gray-100">
            ${contentHTML}
        </div>
    `;
}
...
// [FIXED] 核心读取函数 - 支持单位读取与 Steam/绿电 逻辑
export function readAllInputs(showErrorCallback) {
    ...
    return {
        ...
        // [FIXED] 修正 Steam 的 ID 读取，确保蒸汽方案能被正确计算
...
// [FIXED] 核心渲染函数 (Dashboard Rendering)
export function renderDashboard(results) {
    ...
    // 响应式字号，防止溢出
    const kpiClass = "text-3xl md:text-4xl xl:text-5xl font-black text-gray-800 mt-2 md:mt-3 tracking-tighter truncate";
