// src/js/ui.js
// V19.9: Usage Analytics + Final Credits & Disclaimer
                        ...
                        <div class="bg-gray-50 p-3 rounded-lg flex justify-between items-center border border-gray-200 related-to-${k}">
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <div class="flex items-center">
                                    ...
                                    <span class="text-xs font-bold text-gray-500 ml-1">kgCO₂/${u}</span>
                                </div>
                            </div>
                        </div>`;
                    }).join('')}
...
export function initializeUI(handleCalculate) {
    ...
    setupUnitAutoConverters(); // [内置] 启动单位自动换算
    updateUsageDisplay(); // [新增] 显示初始计数

    const calcBtn = document.getElementById('calculateBtn');
    if (calcBtn) calcBtn.addEventListener('click', () => {
        incrementUsageCounter(); // [新增] 点击计算，计数+1
        handleCalculate();
    });
...
function updateUsageDisplay(count = null) {
    ...
    if (el) el.textContent = `累计运行：${count} 次`;
}
